...
Quick reference for adding new tests. Update this file whenever a new test file is created or a significant pattern changes.
...
├── test_similarity.py # candidate-pairing similarity score (similarity.py)
...
└── test_render.py      # word-level replace rendering, incl. character-level diffing (render.py)
```
...
| `test_align.py` | `align_units` / `align_paragraphs`: identity short-circuit, small in-place edits staying a single `Edit`, totally unrelated text becoming insert+delete, heavy rewrites not crashing and not losing content, paragraph insertion/deletion, empty input, `_align_sequence`'s 1-vs-1 forced-pairing fast path (regression test: a hole with exactly one unit on each side must pair unconditionally, not fall through `PAIR_THRESHOLD`), and a 1-vs-M control confirming the fast path doesn't over-fire when there's more than one candidate on a side. |
| `test_similarity.py` | `similarity_score`: identical/empty/no-overlap edge cases, below-threshold behavior matching plain Jaccard exactly (including the `overlap == PAIR_OVERLAP_THRESHOLD` boundary), genuine containment scoring near 1.0 (the motivating case, versus ~0.19 under plain Jaccard), and a false-positive control confirming short/unrelated text isn't boosted. |
...
| `test_render.py` | `_render_token_pair`/`_render_replace`/`_render_edit`: identical-word no-op, punctuation-only changes marking only the punctuation (regression test for the whole-word-swap bug on trailing quotes/periods), unrelated words falling back to a whole-word swap, mismatched token-count replacements using the whole-span fallback when too dissimilar and a nested character-level diff (via `_char_diff_html`) when the joined spans clear `WORD_CHAR_THRESHOLD`, and two end-to-end `align_units` + `_render_edit` checks reproducing the originally reported punctuation bug. |
...
# Rendering internals
from redline.render import render_html, _render_edit, _render_replace, _render_token_pair, _char_diff_html

# Candidate-pairing similarity
from redline.similarity import similarity_score, PAIR_OVERLAP_THRESHOLD
```

---

*Last updated: 2026-07-08 — added test_similarity.py for the new containment-aware similarity_score(); extended test_render.py for the different-token-count character-diff generalization; extended test_align.py for the 1-vs-1 forced-pairing fix.*
